...
'''
求解导数时，按导数规则，一个元素一个元素的求，找规律，然后变换成矩阵形式，例如求解W的导数时，可以先按下面的例子求w11的导数，然后再求整体W的导数
...
则scores_exp = scores = [[ey11, ey12],
                         [ey21, ey22],
                         [ey31, ey32]] 

loss = -np.log(np.sum(scores_correct, axis=1) / np.sum(scores, axis=1))
     =[[-log(ey12 / (ey11 + ey12))],
       [-log(ey21 / (ey21 + ey22))],
       [-log(ey31 / (ey31 + ey32))]]
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        scores_correct_pre = np.zeros_like(scores)
        scores_correct_pre[range(N), y] = 1
        scores_correct = scores_correct_pre * scores
        ...
        scores[range(N), scores_max_arg] = (-np.sum(scores * (np.ones_like(scores) - dscores_max_pre),axis = 1)).flatten()
        dW2 = a1.T.dot((-scores_correct_exp / scores_sum**2) *
                    scores / -(scores_correct_exp / scores_sum))
        
        #dW2 = a1.T.dot(scores * np.ones_like(scores)) - a1.T.dot(scores * dscores_max_pre) # (H,C)
        #dW2 = a1.T.dot(scores_correct * (scores - scores * dscores_max_pre) + scores * scores_correct)  # (H,C)
        db2 = (np.ones((N, 1)).T).dot((-scores_correct_exp / scores_sum**2) *
                                      scores / -(scores_correct_exp / scores_sum))  # (1,C)
        da1 = scores.dot(W2.T) / scores_sum - W2.T[y,:]  # (N,H)
        dW1 = X.T.dot(da1 * da1_cache)  # (D,H)
        db1 = (np.ones((N,1)).T).dot(da1 * da1_cache) #(1,H)
    ...
    def predict(self, X):
        ...
        scores = (X.dot(self.params['W1']) + self.params['b1']).dot(
                       self.params['W2']) + self.params['b2'] #(N,C)
